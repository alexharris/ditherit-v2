  ...
  <div class="tournament-root">
    ...
    <div v-if="phase === 'tournament'" class="tournament-wrap">
      ...
      <div class="t-progress-row">
        ...
        <button class="zoom-btn" @click="imageZoom = imageZoom === 4 ? 1 : imageZoom * 2">
          🔍 {{ imageZoom }}x zoom
        </button>
      </div>
      ...
      <div v-if="currentMatch" class="match-arena">
        <div class="match-card" @click="pick(currentMatch[0])">
          ...
          <img :src="currentMatch[0].dataUrl" class="match-img" :alt="currentMatch[0].label" :style="{ width: (imageZoom * 100) + '%', imageRendering: 'pixelated' }" />
        ...
        <div class="match-card" @click="pick(currentMatch[1])">
          ...
          <img :src="currentMatch[1].dataUrl" class="match-img" :alt="currentMatch[1].label" :style="{ width: (imageZoom * 100) + '%', imageRendering: 'pixelated' }" />
          <div class="match-pick-btn">✓ Pick this one</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="phase === 'winner'" class="winner-wrap">
      <div class="winner-card">
        ...
        <img :src="winner.dataUrl" class="winner-img" :alt="winner.label" :style="{ width: Math.min(imageZoom * 100, 100) + '%', imageRendering: 'pixelated' }" />
        <div class="zoom-row">
          <button class="zoom-btn" @click="imageZoom = imageZoom === 4 ? 1 : imageZoom * 2">🔍 {{ imageZoom }}x zoom</button>
        </div>
      ...
      <div class="leaderboard-card">
        ...
        <p class="leaderboard-desc">Ranked by how many rounds each variant survived. Click any image to download it.</p>
        <div class="leaderboard-list">
          ...
          >
            ...
            <div class="lb-thumb-wrap">
              <a v-if="entry.dataUrl" :href="entry.dataUrl" :download="'dither_' + (entry.config.algorithm || 'bayer') + '_rank' + (i+1) + '.png'" class="lb-thumb-link">
                <img :src="entry.dataUrl" class="lb-thumb" :alt="entry.label" />
                <span class="lb-thumb-dl">💾</span>
              </a>
              <div v-else class="lb-thumb-empty">freed</div>
            </div>
            <div class="lb-info">
    ...
    <!-- ===================== SOLO RESULT (only 1 image produced) ===================== -->
    <div v-if="phase === 'solo'" class="winner-wrap">
      <div class="winner-card">
        <div class="winner-crown">🖼️</div>
        <h2 class="winner-title" style="font-size:1.6rem;">Only one image was made</h2>
        <p class="winner-label" style="color:#666;font-weight:400;font-size:0.95rem;">Your selected options only produced a single dithered result — there was nothing to compare it against. Here it is, and you can still download it below.</p>
        <p class="winner-label">{{ winner.label }}</p>
        <img :src="winner.dataUrl" class="winner-img" :alt="winner.label" :style="{ width: Math.min(imageZoom * 100, 100) + '%', imageRendering: 'pixelated' }" />
        <div class="zoom-row">
          <button class="zoom-btn" @click="imageZoom = imageZoom === 4 ? 1 : imageZoom * 2">🔍 {{ imageZoom }}x zoom</button>
  ...
  methods: {
    ...
    async renderAllVariants() {
      ...
      if (this.contestants.length === 1) {
        this.winner = this.contestants[0]
        this.totalRounds = 1
        this.phase = 'solo'
        return
      }
    ...
    pick(chosen) {
      ...
      // Do NOT free loser's dataUrl here — we need it for the leaderboard thumbnails
      ...
      if (this.matchIndex >= this.currentPairs.length) {
        if (this.winners.length === 1) {
          ...
          this.phase = 'winner'
          // Don't purge yet — leaderboard needs the dataUrls
        } else {
          this.currentRound++
    ...
    // Called when user is done viewing the leaderboard to free all remaining memory
    purgeLeaderboard() {
      this.contestants.forEach(c => {
        if (c !== this.winner) this.freeLoser(c)
      })
    },
    purgeAll() {
...
.t-progress-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; }
.t-progress-label { font-size: 0.8rem; color: #999; margin: 0; }
.zoom-btn { font-size: 0.78rem; font-weight: 700; background: #fff; border: 2px solid #1a1a1a; border-radius: 2px; padding: 0.25rem 0.6rem; cursor: pointer; font-family: inherit; transition: all 0.15s; }
.zoom-btn:hover { background: #1a1a1a; color: #fff; }
.zoom-row { display: flex; justify-content: center; margin-bottom: 1rem; }
...
.leaderboard-row {
  display: grid; grid-template-columns: 2.5rem 64px 1fr 120px;
