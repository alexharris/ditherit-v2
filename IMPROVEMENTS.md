...
~~`tailwind.css` defines `btn-red`, `btn-red-outline`, `btn-red-small-outline`, `btn-red-large`, `btn-red-large-outline`, `btn-blue`, `btn-green`, `btn-grey` — each with separate hover rules. A few of these (`btn-blue`, `btn-red-large`, `btn-red-large-outline`) don't appear to be used anywhere. The remaining ones could be simplified.~~ Removed unused button classes (`btn-blue`, `btn-red-large`, `btn-red-large-outline`).
...
~~The original file size is estimated from `selectedImage.src.length` (which is a base64 data URL). This is an approximation that includes the data URL prefix and doesn't account for base64 overhead correctly. The actual file size from the File object (`images[i].size`) is available but not used for comparison.~~ Now uses `selectedFile.size` from the actual File object.

### ~~4d. `loading` flag set to `false` too early in `ImageUpload.vue`~~ ✅
~~In both `imageUploaded()` (line 129) and `startWithDuck()` (line 202), `this.loading = false` runs immediately after scheduling `setTimeout` callbacks, so the loading spinner disappears before the images are actually loaded.~~ Fixed with async/await — `loading = false` now runs after `Promise.all()` completes.
...
```
~~This should be `<=` or should check against the current palette length, since it compares against the loop index `i` and will skip storing the first color.~~ Fixed to use `<=`.
...
~~The method is misspelled throughout `ColorPicker.vue`. Not a functional issue but worth fixing for consistency.~~ Fixed to `updatePalette`.
...
```
~~Note: `duck` is currently passed as a string `"true"` and compared with `== 'true'`, which would change to an actual boolean.~~ Converted props to object syntax with type validation in `ImageUpload`, `ColorPicker`, `FilesizeResults`, `Toggler`, `TopToolbar`, and `HelpTooltip`.
